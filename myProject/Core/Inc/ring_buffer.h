...
typedef struct {
	...
	//int TODO;
	char *data; //pointer to a location in memory, where the ring buffer data will be stored
	size_t capacity; //size in bytes of the dataBuffer
	
	size_t head; //indeks gdzie jest dodawany nowy znak
	size_t tail; //indeks skad odczytywany jest znak
	
	bool isFull; //czy pelny
} RingBuffer;
